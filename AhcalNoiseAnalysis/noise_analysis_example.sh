...
for run in `seq 61000 -1 60000`
do
    path=/home/kvas/mount_pnfs/AhcalRaw
    #/data/201805/
    ...
    $gnuplotcmd -e "
           ...
           do for [ls=0:1] {
             ...
             do for [p=0:47] {
                 ...
                 plot 'noise${run}.txt'  u (\$2!=p?NaN:\$4!=0?NaN:floor(getxpos(\$3-1,\$4)/6)):(floor(getypos(\$3-1,\$4)/6)):(0.5):(0.5):(\$8<2?NaN:\$8/\$5) palette w boxxyerrorbars notitle,\
           ...
           do for [ls=0:1] {
             ...
             do for [p=0:47] {
                 set title sprintf(\"port %d\",p) offset 0,-0.8;
                 plot 'noise${run}.txt'  u (\$2!=p?NaN:getxpos(\$3-1,\$4)):(getypos(\$3-1,\$4)):(0.5):(0.5):(\$6<0?NaN:\$6/\$5) palette w boxxyerrorbars notitle\
             };
           ...
           do for [ls=0:1] {
             do for [p=0:39] {
               unset logscale y ; stats 'noise${run}.txt' u (\$2!=p?NaN:\$4) nooutput;
               if (STATS_sum != 0) {
               print sprintf(\"Printing map for port %d\",p);
                 ...
                 plot 'noise${run}.txt'  u (\$2!=p?NaN:getxpos(\$3-1,\$4)):(getypos(\$3-1,\$4)):(0.5):(0.5):(\$6<0?NaN:\$6/\$5) palette w boxxyerrorbars notitle,\
                      '' u (\$2!=p?NaN:getxpos(\$3-1,\$4)):(getypos(\$3-1,\$4)):(sprintf(\"%.2f\",\$6/\$5)) w labels notitle;
...
#gnuplot commands
           ...
           do for [ls=0:1] {
             ...
             do for [p=0:47] {
                 ...
                 plot 'noise${run}.txt' u (\$4!=0?NaN:\$2!=p?NaN:floor(\$3+256-${ASIC_START})%16):(\$8/\$5) w boxes notitle;
             };
             unset multiplot; 
           };
               ...
               if (STATS_sum != 0) {
                 print sprintf(\"Printing histograms for port %d\",p);
                   ...
                   do for [a in \"${ASIC_PATTERN}\"] {
                       ...
                       plot 'noise${run}.txt' u (int(\$3+256-${ASIC_START})%16!=a?NaN:\$2!=p?NaN:\$4):(\$6/\$5) w boxes notitle
                   };
                   unset multiplot;
