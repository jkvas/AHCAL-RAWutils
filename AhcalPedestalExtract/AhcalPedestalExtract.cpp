...
//      { "adc_cut", required_argument, nullptr, 'c' },
      ...
      { "skip_firstfill", required_argument, nullptr, 's' },      //1=reset on run, 2=reset after a delay>1s
      { nullptr, 0, nullptr, 0 } };
...
void PrintHelp () {
   ...
   std::cout << "   -w, --spiroc_raw_filenames" << std::endl;
...
void prefetch_information (char *filename, const struct arguments_t &arguments, std::map<uint32_t, std::map<uint16_t, bool> > &ROCtriggers, std::map<int, u_int64_t> &startTSs,
			   ...
			   std::map<int, u_int64_t> &busyTSs, std::map<int, int> &memcells)
{
   ...
   while (1) {
	 ...
	 TS = (u_int64_t) buf[0] + ((u_int64_t) buf[1] << 8) + ((u_int64_t) buf[2] << 16) + ((u_int64_t) buf[3] << 24) + ((u_int64_t) buf[4] << 32) + ((u_int64_t) buf[5] << 40);
	 switch (type) {
	    case 0x01: //start_acq
	       within_ROC = 1;
	       startTSs[ROcycle] = TS;
...
//   std::map<uint32_t, int> acquisitions; //maps (LDA.Port.chip) to the number of acquisitions
      ...
      std::map<uint32_t, int> occurances; //port..asic..cell
	 ...
	 // }
	 ROcycle = update_counter_modulo (ROcycle, ((headlen >> 16) & 0xFF), 0x100, 100);
	 if (arguments.skip_firstfill >= 2) {
	    try {
	       if ((startTSs.at (ROcycle) - startTSs.at (ROcycle - 1)) > 40 * 1000 * 1000) {
		  std::cout << "#INFO: reseting first occurances in memory cells. ROC=" << ROcycle << ", DIFF=" << (startTSs.at (ROcycle) - startTSs.at (ROcycle - 1)) << "("
			<< (startTSs.at (ROcycle) - startTSs.at (ROcycle - 1)) / 40000000 << " s)" << std::endl;
		  occurances.clear ();
	       }
	    }
	 ...
	 if ((buf[0] == 0x41) && (buf[1] == 0x43) && (!((buf[2] == 0x48) && (buf[3] == 0x41))) && (buf[5] == 0x41) && ((buf[4] == 0x49) || (buf[4] == 0x50) || (buf[4] == 0x51))) {
	 ...
	 if ((buf[0] == 0x41) && (buf[1] == 0x43) && (buf[2] == 0x48) && (buf[3] == 0x41)) { //SPIROC
	    ...
	    if (ROCLength > (65536 * arguments.bxid_length + arguments.correlation_shift)) {
	       std::cout << "#Readout cycle length=" << ROCLength << " too long. ROC=" << ROcycle << " Startroc=" << startTSs[ROcycle] << " StopROC=" << stopTSs[ROcycle] << " ROCLength=" << ROCLength
	    ...
	    for (memcell = arguments.dummy_triggers; memcell < memcell_filled; ++memcell) {
	       bxid = buf[8 + 36 * 4 * memcell_filled + 2 * (memcell_filled - memcell - 1)] | (buf[8 + 36 * 4 * memcell_filled + 2 * (memcell_filled - memcell - 1) + 1] << 8);
		  ...
		  if (occurances[(port << 16) | (asic << 8) | (memcell)] <= 0) {
		     std::cout << "#DEBUG skipped because first ocurance, A=" << asic << ", P=" << (int) port << ", M=" << memcell << std::endl;
		     continue;
		  }

	       for (channel = 0; channel < 36; ++channel) {
		  adc = buf[8 + (35 - channel) * 2 + 36 * 4 * (memcell_filled - memcell - 1) + 36 * 2] | (buf[8 + (35 - channel) * 2 + 36 * 4 * (memcell_filled - memcell - 1) + 36 * 2 + 1] << 8);
		  adc_hit = (adc & 0x1000) ? 1 : 0;
		  if (adc_hit)
		     numOfHits++;
	       }
	       if ((arguments.max_hits_in_memcell > 0) && (numOfHits > arguments.max_hits_in_memcell))
		  continue;
	       for (channel = 0; channel < 36; ++channel) {
		  //  if ((arguments.channel != -1) && (channel != arguments.channel)) continue;/*ship data from unwanted channel*/
		  tdc = buf[8 + (35 - channel) * 2 + 36 * 4 * (memcell_filled - memcell - 1)] | (buf[8 + (35 - channel) * 2 + 36 * 4 * (memcell_filled - memcell - 1) + 1] << 8);
		  adc = buf[8 + (35 - channel) * 2 + 36 * 4 * (memcell_filled - memcell - 1) + 36 * 2] | (buf[8 + (35 - channel) * 2 + 36 * 4 * (memcell_filled - memcell - 1) + 36 * 2 + 1] << 8);
		  adc_hit = (adc & 0x1000) ? 1 : 0;
...
//               if (adc_gain != tdc_gain) mismatches_gain++;
	    ...
	    for (memcell = 0; memcell < memcell_filled; ++memcell)
	       occurances[(port << 16) | (asic << 8) | (memcell)]++;
	 }
...
//      acquisitions[((lda << 16) | (port << 8) | (asic & 0xFF))]++;
   ...
   std::cout << "#Port\tAsic\tChan\tCell\tMax\tMaxAt\tMean10\tStddev10\tcount10\tMean100\tStddev100\tcount100\tMean1000\tStddev1000\tcount1000\t#pedstats" << std::endl;
